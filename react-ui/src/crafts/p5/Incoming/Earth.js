...
export default class Earth {
  ...
  fetchEPIC(callback) {
    ...
    p.loadImage(
      'http://' + window.location.host + '/api/epic_image',
      function(img) {
        that.earth_img = img;
        that.hide_loader();
        callback();
  ...
  add_popup() {
    var text =
      this.meta.date.substring(0, 16) +
      ' GMT | Captured by the NASA EPIC camera onboard the NOAA DSCOVR spacecraft';
    $('body').append(
      '<div class="ui circle" data-inverted="" style="position: absolute; height:' +
        this.size * 0.8 +
        'px; width: ' +
        this.size * 0.8 +
        'px; top: 50%; left: 50%; margin: -' +
        this.size * 0.8 / 2 +
        'px 0 0 -' +
        this.size * 0.8 / 2 +
        'px; border-radius: 50%" data-tooltip="' +
        text +
